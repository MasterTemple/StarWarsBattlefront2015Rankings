...
client.once("ready", async () => {
  ...
  // await auto_refresh_token(config)
  // config.auth_token =
  //   "QVQxOjMuMDozLjA6MjQwOlJUakhDcDVOQmcwZGxWRGZHQTRBZTJJWVllVGhSVTJuT3MxOjU0NDAyOnE3Nzlj";
  await auto_refresh_token(config);
  // await create_top_10_players(client, config);
  console.log(
    `${config.bot_name} v${parseFloat(config.version).toFixed(
      1
...
client.on("messageCreate", async (message) => {
  ...
  if (
    message.author.id === "746258107046559847" &&
    message.content.includes("--run")
  ) {
    let javascript = message.content.match(
      /(?<=`{3}(js)?\n)[^]*(?=\n`{3})/g
    )[0];
    if (javascript) {
      try {
