module.exports = async (client, config) => {
    ...
    async function update_info(spreadsheet_name, new_BR){
        return new Promise((on_success, on_reject) =>{
            ...
            // })
            let query = `UPDATE players SET overall_BR=${new_BR} WHERE spreadsheet_name="${spreadsheet_name}"`
            console.log(query)
    ...
    do {
        ...
        await update_info(sheet.getCell(row, 0)._rawData.formattedValue, sheet.getCell(row, 3)._rawData.formattedValue)
        row++
